...
/// \file testTrackletTransformer.cxx
/// \brief This task tests the Tracklet Transformer functions
...
{
  std::cout << "c: " << calculatedPoint << " | "
            << "p: " << predictedPoint << std::endl;
...
BOOST_AUTO_TEST_CASE(LocaltoRCTest)
{
  ...
  // std::cout << TMath::Tan(TMath::DegToRad()*tiltingAngle) * 0.5 * lengthIPad / padIWidth << std::endl;
  ...
  // Test pads without pad tilting
  // auto p7 = padPlane->getPad(0);
  // // Center of the chamber. This should return the lower edge of pad 72.
  // // Since we are using float values, the lower edge of pad 72 correspond with float value of 72.0.
  // testRCPoint(p7, 72);

  // auto p8 = padPlane->getPad(padIWidth / 2.);
  // // Move forward 1/2 of inner pad width. Pad number increases in positive y direction so should get 72.5
  // testRCPoint(p8, 72.5);

  // auto p9 = padPlane->getPad(-padIWidth / 2.);
  // // Move backward 1/2 inner pad width
  // testRCPoint(p9, 71.5);

  // auto p10 = padPlane->getPad(padIWidth * 7.77);
  // // Move an arbitrary distance in y
  // testRCPoint(p10, 72 + 7.77);

  // auto p11 = padPlane->getPad(padIWidth * 71 + padOWidth);
  // // Top border case right on the edge of the chamber
  // testRCPoint(p11, 144);

  // auto p12 = padPlane->getPad(-padIWidth * 71 - padOWidth);
  // // Lower border case
  // testRCPoint(p12, 0);

  // Test pads with pad tilting
  double p13 = padPlane->getPad(0, 0.01);
  // Center of chamber. Since there is a discontinuity in pad vs. z at z=0, we place the point slightly above
  // the center (in z direction). This puts us at a point on the lower (in z direction) end of padrow 7.
  // Since we have a pad tilt of -2 deg (pads tilted clockwise) we expect to be a bit into pad 72.
  ...
  testRCPoint(p13, 72 + TMath::Tan(TMath::DegToRad() * tiltingAngle) * (lengthIPad - 0.01 - 0.5 * lengthIPad) / padIWidth);

  double p14 = padPlane->getPad(0, -lengthIPad / 2.);
  // Move from center of chamber to to center of padrow 8 = 8.5.
  // This should now place us as edge of pad 72 = 72.0
  testRCPoint(p14, 72);
  ...
  // a padrow, but rather 2.3 cm into padrow 8 + 4 = 12. This puts us below the center of the padrow which is 4.5 cm
